...
WHATSAPP_API_URL=your_whatsapp_api_url   # e.g. https://graph.facebook.com/v20.0/<phone_number_id>/messages
...
1) Start the LLM + media agent (used by the webhook) on port 5000:

```bash
uvicorn ec2_endpoints:app --host 0.0.0.0 --port 5000
```

2) In another shell, start the WhatsApp webhook listener (set your public HTTPS URL to this service in Meta):

```bash
uvicorn webhook_main:app --host 0.0.0.0 --port 8000
```

Make sure the ports you choose are open in your EC2 security group, and that `AGENT_URL` points to the running `ec2_endpoints` service.
...
/Users/sam/Documents/Ceptes/whatsapp_flow_llama-cookbook-main/.venv/bin/python -m uvicorn ec2_endpoints:app --host 0.0.0.0 --port 5000
Read whatsapp_llama_4_bot

/Users/sam/Documents/Ceptes/whatsapp_flow_llama-cookbook-main/.venv/bin/python -m uvicorn webhook_main:app --host 0.0.0.0 --port 8000

/Users/sam/Documents/Ceptes/whatsapp_flow_llama-cookbook-main/.venv/bin/python -m uvicorn ec2_endpoints:app --host 0.0.0.0 --port 5000 --reload