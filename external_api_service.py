...
class ExternalApiService:
    ...
    @staticmethod
    def send_media(media_bytes, filename, mimetype, message_id, phone_number, media_type):
        files = {
        ...
        try:
            logger.info(f"Sending media to API: {filename} {mimetype} {data}")
            resp = requests.post(API_URL, headers=headers, data=data, files=files, timeout=60)
            logger.info(f"API response: {resp.status_code} {resp.text}")
            if resp.status_code == 200:
                api_data = resp.json() if resp.headers.get("content-type", "").startswith("application/json") else {"reply": resp.text}
                return api_data.get("reply") or resp.text
            else:
                logger.error(f"API error: {resp.status_code} {resp.text}")
                return None
        except Exception as exc:
